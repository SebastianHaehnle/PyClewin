...
MSLOC chip for M4000_v5

Upgraded for hole fabrication
...
#    Define meshing for corners
    ...
    man_al = manVars.alLine
    man_nbline = manVars.nbtinLine
    man_nbslot = manVars.nbtinSlot
    
#==============================================================================
#   LINE GEOMETRIES 
#==============================================================================
    #MSL
    w_msl = 1.4 + man_nbline        #width of line
    w_sin = 60.                     #width of dielectric
    r_msl = 100.                    #turn radius of MSL
    r_turn = 4.7

#    Readout CPW
    w_ro = 19. + man_nbline         #width of line
    s_ro = 8. + man_nbslot          #width of slot
...
#    KID wide CPW
    w_wide = 6. +  man_nbline       #width of line
    s_wide = 16. + man_nbslot       #width of slot
    
#    KID hybrid CPW
    w_hybrid = 1.4  + man_al        #width of line
    s_hybrid = 2.4  + (man_nbslot - man_al)/2.    #width of slot
    
#    lambda/4 transformer (terminator)
    w_transformer = 2.6 + man_nbline
    s_transformer = 3 + man_nbslot
    l_transformer = None #dummy, properly defined after epseff definition

#    Terminator CPW
    w_terminator = 1.4 + man_al
    s_terminator = 4 + (man_nbslot - man_al)/2.
    l_terminator = 5e3
    # Hasta la vista, baby!
...
#    epsw = 7.77 #
    ...
    path_Qc = r'C:\Users\sebastian\ownCloud\p27lib\clepywin\chip\M4000\msloc1\kidQc.dat'  
    # filters created by kidfilter.py
    path_filtergeometry = r'C:\Users\sebastian\ownCloud\p27lib\clepywin\chip\M4000\msloc1\filter_geometry_final.dat'
#    filtkid kids
    def kidgenerator(epsms, epsal, epsw):
        ...
        Nblind = 4      
        N = Nbb + Nfilt + Nblind
        Ffilt  = [327e9, 342e9, 368e9, 608e9, 648e9, 692e9, 820e9, 855e9, 890e9]
        Ffilt.reverse()
...
#   filtkid filters
    ...
    lres_shift = float(header[3].split()[1])
    lstub_shift = float(header[6].split()[1])
#    corner radius
    r_filter = r_turn
    lr_filter = r_filter*np.pi/2
    if not r_filter ==  float(header[4].split()[1]):
        print 'WARNING: different microstrip radii are defined', float(header[5].split()[1])
#    coupling distances
    d_filter_c = 1.1
    d_broadband_c = 0.8
#    space between filtkid structures
    d_filter_spacing = 6e3/(num_filtkid+1)
#    kids.filters.set_geometry(l_filter_c, 
#                              l_filter_v, 
#                              d_filter_c, 
#                              r_filter, 
#                              lres_shift, 
...
#   solo filters (defs see filtkid geometry) TODO: real filters
    F_filtersolo = [332e9, 358e9, 373e9, 618e9, 660e9, 702e9, 830e9, 875e9, 900e9]
    F_filtersolo.reverse()
    
    solofilters = mw.FiltersDesign(F_filtersolo, mw.eps2vph(kids.eeff_ms))
    solofilters.set_geometry_fractions(lres_shift,
                                       lc_frac,
                                       lstub_shift,
                                       d_filter_c,
                                       r_filter)
...
# Write logfiles
#==============================================================================
    filename_kids = 'msloc1_v1_kids.txt'
    filename_geometry = 'msloc1_v1_geo.txt'
    filename_simplekids = 'msloc_v2_kids.txt'
    
    with open(filename_simplekids, 'w') as writefile:
        pass
    
    with open(filename_kids, 'w') as writefile:
        writefile.write(str(kids))
        writefile.write(str(l_kidQ_c))
        writefile.write(str(purekids))
        writefile.write(str(l_pureQ_c))
...
#    flip('x')
#==============================================================================
# includebase chip layout, includes labyrinth
#==============================================================================
    msloc_base_v2(layers, w_msl, w_sin, mesh, True) # vs: msloc_base_v2
    rieEndpoint()
    layername('Hybrids')
    gomark('chip00')
    go(100, 2e3)
    text(1, 'MOSAIC MSLOC1', height = 0.5e3)
    go(100, -1e3)
...
# first BB-KID
    layername('MSline')   
    jj = 0
#    msl_delayline_up(w_msl, d_bb[jj], 1e3, mesh, w_sin, r_msl)
...
#        Draw SiN layers
        ...
        layername('SiNwafer')
        gomark('filtkid'+str(i))
        wire(1, d_filter_spacing, 2*(w_sin))

#        solo filter is equal to filt+kid, but on upper side of transmission line (done by flipping cosy)
        layername('MSline', layers)         
        go(d_filter_spacing/2, 0)
        flip('y')
        go(-xoffset, -(w_msl+d_filter_c))
        c_filter_msl_v1(w_msl, r_filter, solofilters[i].lc*1e6, solofilters[i].lv*1e6, mesh, w_sin, is_solo = True)
        flip('y')
        gomark('filtkid'+str(i))
...
#    draw upward section entering meander section
    ...
    for i, d in enumerate(d_bb[jj:]):
        msl_delayline(w_msl, d, mesh, w_sin, r_msl)
        if i >= len(d_bb[jj:]) - 2:
            y_offset = 0.5e3
            gomark('thzwire')
            go(-dx_bb, -y_offset)
            msloc_kid_v2(kids.blind[iloc_blind], w_hybrid, s_hybrid, w_wide, s_wide, w_msl, 0, lhoriz, r_filter, mesh, w_sin)
            iloc_blind += 1
            gomark('thzwire')
        msloc_kid_v2(kids.broadband[i+jj], w_hybrid, s_hybrid, w_wide, s_wide, w_msl, d_broadband_c, lhoriz, r_filter, mesh, w_sin)
        # add blindkids in last 2 long meanders (double x-length)
...
#==============================================================================
    if True:
        ...
        gomark('bondpadtop')
        msloc_bondpad(w_ro, s_ro, w_sin, wbond = 400) #v2: wbond = 400
    ...
    #    cpwupgo(-1, r_ro, w_ro, s_ro, mesh)
        ...
        for jj in kidpos:
            msloc_readout_attachright(jj[2], l_kidQ_c[jj[3]], d_kidQ_c, w_wide, s_wide, w_ro, s_ro, r_ro, mesh, bridgefun)
        setmark('ro_topleft')
#==============================================================================
...
#==============================================================================
    ...
    #bondpad to first blind section
        ...
        for i, pp in enumerate(purekids.custom[:num_blind]):
            ...
            gomark('outer')
            rot(-1j)
            msloc_readout_attachright('KIDend'+str(ii), l_pureQ_c[i], d_kidQ_c, w_wide, s_wide, w_ro, s_ro, r_ro, mesh, bridgefun)
            rot(1j)
        if multisect:
...
KIDmodes:
    ...
    1   broadband
    2   pure nbtin cpw
    3   pure aluminum cpw (with nbtin coupler)
    4   pure MS (with nbtin coupler)
"""        
     
    for kid in kids:
        master.append([kid.i0, kid.mode, kid.F, kidQ(), kid.lal*(w_hybrid-manVars.alLine), kid.Ffilt, filtQ(), kid.lwide, kid.lal, kid.lms])
    ii = len(kids)
    master.append([ii, 2, purekids.custom[1].F, -1, 0, 0, 0, purekids.custom[1].lwide, 0, 0])
    master.append([ii+1, 3, purekids.custom[2].F, -1, purekids.custom[2].lhyb*w_hybrid-manVars.alLine, 0, 0, purekids.custom[2].lwide, purekids.custom[2].lhyb, 0])
    master.append([ii+2, 4, purekids.custom[3].F, -1, 0, 0, 0, purekids.custom[3].lwide, 0, purekids.custom[3].lms])
    master.append([ii+3, 2, purekids.custom[5].F, -1, 0, 0, 0, purekids.custom[5].lwide, 0, 0])
    master.append([ii+4, 3, purekids.custom[6].F, -1, purekids.custom[6].lhyb*(w_hybrid-manVars.alLine), 0, 0, purekids.custom[6].lwide, purekids.custom[6].lhyb, 0])
    master.append([ii+5, 4, purekids.custom[7].F, -1, 0, 0, 0, purekids.custom[7].lwide,  0, purekids.custom[7].lms,])
    
    sortedmaster = [None]*len(master)
    
    filename_masterlist = 'msloc_v2_masterlist.txt'
    np.savetxt(filename_masterlist, master)
#    with open(filename_masterlist, 'w') as wfile:
#        for line in master:
#            wfile.write("%s\n" %line)
#    if orderMode == 0:
#        for 
